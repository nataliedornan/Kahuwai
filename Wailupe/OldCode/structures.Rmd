...
#arrange columns
junctions <- conduits %>% select( Nodes , Elevation,  MaxDepth, InitDepth, SurDepth, Aponded) %>% mutate(MaxDepth = ifelse(is.na(MaxDepth), 0, MaxDepth)) %>% mutate(Aponded = ifelse(is.na(Aponded), 0, Aponded)) %>% distinct()

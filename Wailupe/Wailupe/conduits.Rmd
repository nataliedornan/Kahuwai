...
conduits <- conduits_data %>% 
  select( OBJECTID, ROUGHNESS, OBJECTID_12, RASTERVALU_1, TYPE, POINT_X, POINT_Y, SLOPE, DIAMETER, WIDTH, HEIGHT) %>% rename(Name = OBJECTID, Roughness = ROUGHNESS, Nodes = OBJECTID_12, Elevation = RASTERVALU_1) 
...
distinct_conduits_1 <- length %>% mutate(Roughness = if_else(is.na(Roughness), 0.02, Roughness)) %>%  filter(Nodes2 !="NA") %>% filter(Nodes1 !="NA") %>% select(Name, Nodes1, Nodes2, length, Roughness, InOffset, OutOffset, InitFlow, MaxFlow, TYPE, DIAMETER, WIDTH, HEIGHT, Elevation1, Elevation2) %>% distinct() 

...
2. For conduits where slope was provided, I took an average of the slopes: 59 conduits with a slope sum of 6.9443 divided by 59 = 0.1177. I will use these values to replace any NA values

3. Using the slope and distance we will find the length of the conduit
...
Alternate step 2/3 : use difference in elevation to calculate height, then length
```{r}
...
I calculated average roughness value to be 1.18, so replace all "NA" values with 1.18. Also filter any conduits that do not have two nodes assigned to them - SWMM will not take these conduits, then select the rows you want to use for the two file outputs needed for SWMM.


```{r}

# data frame for both file outputs:
distinct_conduits_1 <- length %>% mutate(Roughness = if_else(is.na(Roughness), 1.18, Roughness)) %>%  filter(Nodes2 !="NA") %>% filter(Nodes1 !="NA") %>% select(Name, Nodes1, Nodes2, length, Roughness, InOffset, OutOffset, InitFlow, MaxFlow, TYPE, Elevation1, Elevation2,) %>% 
  distinct() 
...
Condiut file output:
```{r}
write.csv(distinct_conduits_1,"Wailupe_conduits_SWMM.csv", row.names = FALSE)
```
...
```{r}
#for conduits, use Type to populate the Shape column that SWMM takes. Rename the types byt the different shapes provided by SWMM

...
hotspots_dry_total <- tm_basemap("OpenStreetMap.Mapnik") +
  tm_shape(subcatch_dry) +
  tm_polygons("runoff_coeff", alpha = 0.8, palette = "Blues", style = "cont", n=8, legend.hist = TRUE, title = "Runoff Coefficient") +
  tm_layout(inner.margins=c(.05, .05, .05, .47), legend.position =  c("right","top"), legend.title.size = 1.4, legend.text.size = 1) +
...
hotspots_dry_peak <- tm_basemap("OpenStreetMap.Mapnik") +
  ...
  tm_layout(inner.margins=c(.05, .05, .05, .47), legend.position =  c("right","top"),legend.title.size = 1.4, legend.text.size = 1, legend.width  = 1) +
  tm_text("subcatchment", size = 0.3)
hotspots_dry_peak
...
#Top 15 of total volume
top15_dry_total <- subcatch_dry %>% 
  arrange(-runoff_coeff) %>% 
  head(15)

top15_wet_total <- subcatch_wet %>% 
  arrange(-runoff_coeff) %>% 
  head(15)

common_total_vector <- as.data.frame(intersect(top15_dry_total$subcatchment, top15_wet_total$subcatchment)) 
colnames(common_total_vector) <- c("subcatchment")


common_total <- subcatch_dry %>% 
  mutate(hotspot = case_when(subcatchment == "45"| 
  subcatchment =="47"|
  subcatchment == "60" |
  subcatchment == "67" | 
  subcatchment == "71"| 
  ...
  subcatchment == "29"| 
  subcatchment == "68" | 
  subcatchment == "21" ~ "Hotspot"))

common_total$hotspot <- as.factor(common_total$hotspot)

top15_total_map <- tm_basemap("Hydda.Base") +
  tm_shape(common_total) +
  tm_polygons("hotspot", title = "Legend", textNA = "Subcatchment", palette="#045a8d", alpha = 0.8)+
  tm_layout(inner.margins=c(.05, .05, .05, .47), legend.position =  c("right","top"),legend.title.size = 1.4, legend.text.size = 1, legend.width  = 1) +
  tm_text("subcatchment", size = 0.3)
top15_total_map

tmap_save(top15_total_map, "top15_total_map.png")

#Top 15 of peak volume

top15_dry_peak <- subcatch_dry %>% 
  arrange(-peak_runoff_cfs) %>% 
  head(15)

top15_wet_peak <- subcatch_wet %>% 
  arrange(-peak_runoff_cfs) %>% 
  head(15)

common_peak_vector <- as.data.frame(intersect(top15_dry_peak$subcatchment, top15_wet_peak$subcatchment)) 
...
top15_peak_map <- tm_basemap("Hydda.Base") +
  tm_shape(common_peak) +
  tm_polygons("hotspot", title = "Legend", textNA = "Subcatchment", palette="#006d2c", alpha = 0.8)+
  tm_layout(inner.margins=c(.05, .05, .04, .4), legend.position =  c("right","top"), legend.title.size = 1.4, legend.text.size = 1, legend.width  = 1) +
  tm_text("subcatchment", size = 0.3)
top15_peak_map

tmap_save(top15_peak_map, "top15_peak_map.png")

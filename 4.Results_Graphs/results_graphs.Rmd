...
**NOTE: Graphs will not be shown in knitted document
...
```{r, fig.width=10, fig.height=5, results= 'asis', fig.show='hide', message = FALSE}
...
write.csv(results_dry, file = "results_dry.csv") ##save for use with graph maps
...
min_range_runoff_table_dry<- results_dry %>% 
  ...
  kable(col.names=c("Minimum Runoff Coefficient", "Maximum Runoff Coefficient", "Median Runoff Coefficient", "Minimum Impervious Runoff (in)", "Maximum Impervious Runoff (in)", "Median Impervious Runoff (In)", "Minimum Pervious Runoff (in)", "Maximum Pervious Runoff (in)", "Median Pervious Runoff (in)")) %>% 
  kable_styling(bootstrap_options = "striped")
...
write.csv(dry_runoff_coef_top_10, file = "dry_runoff_top_15.csv", row.names = FALSE)

write.csv(wet_runoff_coef_top_10, file = "wet_runoff_top_15.csv", row.names = FALSE)

write.csv(dry_peak_top_10,file = "dry_peak_top_15.csv", row.names = FALSE)

write.csv(wet_peak_top_10, file = "wet_peak_top_15.csv", row.names = FALSE)
...
sediment_flow_graph#still kinda ugly to look at, not sure it's worth all the trouble to overlay them on one graph
...
sediment_graph<- sediment09 %>% 
  ...
  geom_segment(aes(x=time_step, xend=time_step, y=0, yend=susp_sed), size=1.5, color="darkgoldenrod1")+
